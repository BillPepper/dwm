...
#define S_FORMAT_CLEAR "sed 's/^..//'"
  ...
  {
    systemReturn = system("slock & sleep .5; xset dpms force off");
  }
  ...
  if (strcmp(exit_action, S_OFFSCREEN) == 0)
  {
    systemReturn = system("sleep .5; xset dpms force off");
  }
  if (strcmp(exit_action, S_EXIT) == 0)
  {
    quit(&(const Arg){0});
  }
  if (strcmp(exit_action, S_REBOOT) == 0)
  {
    systemReturn = system("systemctl reboot");
  }
  if (strcmp(exit_action, S_SHUTDOWN) == 0)
  {
